...
function isPalindrome(str) {
  const strChars = str.toLocaleLowerCase().replace(/[^a-z0-9]/g, '');
  return strChars === strChars.split('').reverse().join('');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (upperCaseChars.includes(str[i])) {
      const index = upperCaseChars.indexOf(str[i]);
      const indexShifted = index < 13 ? index + 13 : index - 13;
      res += upperCaseChars[indexShifted];
    } else if (lowerCaseChars.includes(str[i])) {
      const index = lowerCaseChars.indexOf(str[i]);

      const indexShifted = index < 13 ? index + 13 : index - 13;
      res += lowerCaseChars[indexShifted];
    } else {
      res += str[i];
    }
  }
...
function getCardId(value) {
  ...
  return (
    cardChars.indexOf(value.slice(0, -1)) +
    cardTypes.indexOf(value.slice(-1)) * 13
  );
}
